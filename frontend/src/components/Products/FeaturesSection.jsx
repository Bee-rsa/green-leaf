...
// ─────────────────────────────────────────────────────────────────────────────
// Mobile Reviews
// Matched length so the carousel remains the same height when switching.
// Desktop reviews above remain unchanged.
// ─────────────────────────────────────────────────────────────────────────────
...
const mobileReviews = [
  {
    ...
    rating: 5,
    date: "August 2026",
    text: "Greenleaf Cannabis Club Salt Rock is one of the best cannabis clubs I've visited. The staff are friendly, knowledgeable, and always happy to help. The selection is excellent, the quality is consistent, and the store has a clean, welcoming atmosphere. Every visit has been a great experience.",
  },
  {
    name: "Devon",
    rating: 5,
    date: "January 2026",
    text: "Absolutely amazing experience from start to finish. The customer service is excellent and the staff really know their products. Coming from Johannesburg, I was impressed by the quality and welcoming atmosphere. I couldn't recommend Greenleaf enough and will definitely be back.",
  },
  {
    name: "Jonathan Trenor",
    rating: 5,
    date: "March 2026",
    text: "Great selection and genuinely friendly staff. The store has a relaxed atmosphere and the team are always happy to help with recommendations. The products are consistently good and the overall experience makes it easy to keep coming back. Definitely worth visiting.",
  },
];
  ...
  <div className="flex gap-1 mb-4">
    {Array.from({ length: 5 }).map((_, i) => (
      <HiStar
        key={i}
        className={`text-sm ${
          i < rating ? "text-wood" : "text-sage/30"
        }`}
      />
    ))}
  </div>
...
const ReviewCard = ({ review, className = "" }) => (
  <div
    className={`bg-sand/10 border border-sand/20 rounded-sm px-6 py-7 flex flex-col ${className}`}
  >
    <StarRating rating={review.rating} />

    <p
      className="font-subheading text-base leading-relaxed flex-1 italic"
      style={{ color: "#F2EFE4" }}
    >
      "{review.text}"
    </p>
    ...
    <div className="mt-6 pt-4 border-t border-sand/20">
      ...
      <p
        ...
        style={{ color: "rgba(242,239,228,0.6)" }}
      >
...
const MobileCarousel = () => {
  ...
  const next = () =>
    goTo(
      current === mobileReviews.length - 1
        ? 0
        : current + 1,
      "left"
    );
  ...
  return (
    <div className="xl:hidden">
      ...
      <div
        ...
        style={slideStyle}
      >
        <div className="h-[360px] sm:h-[330px]">
          <ReviewCard
            review={mobileReviews[current]}
            className="h-full"
          />
        </div>
      </div>
